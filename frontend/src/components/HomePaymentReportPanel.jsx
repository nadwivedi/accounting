...
import { CreditCard, IndianRupee, Wallet, Pencil } from 'lucide-react';
...
export default function HomePaymentReportPanel() {
  ...
  const getPartyName = (payment) => {
    if (payment?.party && typeof payment.party === 'object') {
      return String(payment.party.name || payment.party.partyName || payment.party.customerName || '').trim() || '-';
    }
    ...
    return partyNameMap.get(String(payment?.party || '')) || '-';
  };

  const totalAmount = sortedPayments.reduce((sum, payment) => sum + Number(payment.amount || 0), 0);
  const visiblePayments = sortedPayments.slice(0, 8);
    ...
    <section className="w-full rounded-[28px] border border-slate-200/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(241,245,249,0.96))] shadow-[0_28px_70px_rgba(15,23,42,0.18)]">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <StatCard title="Payments" value={sortedPayments.length} icon={CreditCard} tone="from-rose-500 to-pink-500" />
          <StatCard title="Paid Total" value={formatCurrency(totalAmount)} icon={IndianRupee} tone="from-emerald-500 to-teal-500" />
          <StatCard title="Methods" value={new Set(sortedPayments.map((payment) => payment.method || '-')).size} icon={Wallet} tone="from-amber-500 to-orange-500" />
        </div>

        <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white">
          {loading ? (
            <div className="px-4 py-12 text-center text-sm font-medium text-slate-500">Loading payment report...</div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {visiblePayments.map((payment) => (
                    <tr key={payment._id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-sm font-semibold text-slate-700">{payment.voucherNumber || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-700">{getPartyName(payment)}</td>
                      <td className="px-4 py-3 text-sm text-slate-700">{payment.method || '-'}</td>
                      <td className="px-4 py-3 text-right text-sm font-semibold text-emerald-700">{formatCurrency(payment.amount)}</td>
            ...
            <div className="flex flex-col items-center px-4 py-14 text-center">
              ...
              <p className="mt-4 text-base font-semibold text-slate-700">No payment report data found</p>
            </div>
          )}
        </div>
      </div>
