          ...
          <div className='bg-white rounded-2xl shadow-lg border border-gray-200 p-4'>
            <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:items-stretch'>
              <div className='bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl px-3 py-2 border border-blue-200'>
                ...
                <p className={`truncate text-lg font-bold ${closingBalance >= 0 ? 'text-blue-700' : 'text-red-700'}`}>
              ...
              <div className='bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl px-3 py-2 border border-orange-200'>
                ...
                <div className="flex justify-between items-baseline">
                  ...
                  <p className='text-[10px] text-orange-600 font-bold'>Qty: {formatQuantity(summary.saleQty)}</p>
                </div>
              </div>
              ...
              <div className='bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-xl px-3 py-2 border border-cyan-200'>
                <p className='text-[10px] text-cyan-600 font-bold uppercase'>Party Paid Me</p>
                <p className='truncate text-lg font-bold text-cyan-700'>{formatCurrency(summary.totalReceipts)}</p>
              </div>
            </div>
          </div>
        </div>

      <div className="hidden mb-4 bg-white rounded-2xl shadow-lg border border-gray-200 px-4 py-3">
              ...
              {sortedLedgerRows.map((row, index) => {
                ...
                return (
                  <article
                    ...
                    className="rounded-2xl border border-slate-200/60 bg-gradient-to-b from-white to-slate-50/50 p-4 shadow-sm transition-all hover:shadow-md hover:border-slate-300/60"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-col items-start">
                          <span className="text-xs font-medium text-slate-500">{formatDate(row.date)}</span>
                      ...
                      <div className="text-right">
                        {['sale', 'cash sale', 'credit sale', 'sale_cash', 'sale_partial', 'sale_credit', 'purchase', 'cash purchase', 'credit purchase', 'purchase_cash', 'purchase_partial', 'purchase_credit', 'cash', 'partial', 'credit'].includes(row.type) ? (
                          <div className="space-y-1 rounded-lg bg-white/80 px-3 py-2">
                            ...
                            <p className="text-[11px] text-slate-500">Bal: <span className={`font-bold ${Number(row.balance ?? row.impact ?? 0) > 0 ? 'text-rose-600' : Number(row.balance ?? row.impact ?? 0) < 0 ? 'text-purple-600' : 'text-emerald-600'}`}>
